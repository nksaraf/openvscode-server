...
import {
	...
	IExtensionReference,
} from 'vs/workbench/services/extensions/node/extensionPoints';
import {
	IGetEnvironmentDataArguments,
	IRemoteAgentEnvironmentDTO,
	IScanExtensionsArguments,
	IScanSingleExtensionArguments,
...
export const IRawURITransformerFactory = createDecorator<IRawURITransformerFactory>(
	'rawURITransformerFactory',
);

const APP_ROOT = dirname(FileAccess.asFileUri('', require).fsPath);
const uriTransformerPath = path.join(APP_ROOT, 'out/serverUriTransformer');
const rawURITransformerFactory: IRawURITransformerFactory = <any>(
	require.__$__nodeRequire(uriTransformerPath)
);

const WEB_MAIN = path.join(
	APP_ROOT,
	'out',
	'vs',
	'server',
...
async function serveFile(
	...
	try {
		...
		responseHeaders['Content-Type'] =
			textMimeType.get(path.extname(filePath)) || getMediaMime(filePath) || 'text/plain';
...
async function handleCallback(
	...
	const [requestId, vscodeScheme, vscodeAuthority, vscodePath, vscodeQuery, vscodeFragment] =
	...
	mapCallbackUriToRequestId.set(
		requestId,
		JSON.stringify({
			scheme: vscodeScheme || product.urlProtocol,
	...
	return serveFile(
		...
		req,
		res,
		FileAccess.asFileUri('vs/code/browser/workbench/callback.html', require).fsPath,
		{ 'Content-Type': 'text/html' },
	);
}

async function handleFetchCallback(
	req: http.IncomingMessage,
	res: http.ServerResponse,
	parsedUrl: url.UrlWithParsedQuery,
) {
...
export interface IServerOptions {
	...
	start?(
		...
		channelServer: IPCServer<RemoteAgentConnectionContext>,
	): IStartServerResult | void;

	configureExtensionHostForkOptions?(
		opts: cp.ForkOptions,
		accessor: ServicesAccessor,
		channelServer: IPCServer<RemoteAgentConnectionContext>,
	): void;
	configureExtensionHostProcess?(
		extensionHost: cp.ChildProcess,
		accessor: ServicesAccessor,
		channelServer: IPCServer<RemoteAgentConnectionContext>,
	): IDisposable;
...
export async function main(options: IServerOptions): Promise<void> {
	...
	parsedArgs['user-data-dir'] =
		process.env.VSCODE_AGENT_FOLDER || path.join(os.homedir(), product.dataFolderName);
	...
	const channelServer = new IPCServer<RemoteAgentConnectionContext>(
		onDidClientConnectEmitter.event,
	);
	...
	const extraDevSystemExtensionsRoot = path.normalize(
		path.join(rootPath, '..', '.build', 'builtInExtensions'),
	);
	...
	class RemoteExtensionsEnvironment implements IServerChannel<RemoteAgentConnectionContext> {
		...
		async call(
				...
				return transformOutgoingURIs(
					{
						...
						extensionHostLogsPath: URI.file(
							path.join(
								environmentService.logsPath,
								`extension_host_${this.extensionHostLogFileSeq++}`,
							),
						),
			...
			if (command === 'scanExtensions') {
				...
				// TODO: read built nls file
				const translations = {};
				let pendingSystem = ExtensionScanner.scanExtensions(
				...
				if (devMode && builtInExtensions && builtInExtensions.length) {
					pendingSystem = ExtensionScanner.mergeBuiltinExtensions(
						pendingSystem,
							...
							logger,
							{
								resolveExtensions: () => {
									...
									for (const extension of builtInExtensions) {
										result.push({
											name: extension.name,
											path: path.join(extraDevSystemExtensionsRoot, extension.name),
										});
									}
				...
				if (args.extensionDevelopmentPath) {
					pendingDev = args.extensionDevelopmentPath.map((devPath) =>
				...
				const skipExtensions = new Set<string>([
					...args.skipExtensions.map(ExtensionIdentifier.toKey),
	...
	class RemoteFileSystem implements IServerChannel<RemoteAgentConnectionContext> {
		protected readonly watchers = new Map<
			string,
			{
		...
		async call(
			...
			if (command === 'open') {
				const uriTranformer = new URITransformer(rawURITransformerFactory(ctx.remoteAuthority));
				return diskFileSystemProvider.open(
			...
			if (command === 'readFile') {
				const uriTranformer = new URITransformer(rawURITransformerFactory(ctx.remoteAuthority));
				const data = await diskFileSystemProvider.readFile(
					URI.revive(uriTranformer.transformIncoming(arg[0])),
				);
			...
			if (command === 'writeFile') {
				...
				await diskFileSystemProvider.writeFile(
					URI.revive(uriTranformer.transformIncoming(arg[0])),
					data.buffer,
					arg[2],
				);
			...
			if (command === 'delete') {
				const uriTranformer = new URITransformer(rawURITransformerFactory(ctx.remoteAuthority));
				await diskFileSystemProvider.delete(
					URI.revive(uriTranformer.transformIncoming(arg[0])),
					arg[1],
				);
				return;
			...
			if (command === 'watch') {
				...
				if (watcher) {
					...
					const unwatch = watcher.watch(
						URI.revive(uriTranformer.transformIncoming(arg[2])),
						arg[3],
					);
					this.watchHandles.set(arg[0] + ':' + arg[1], unwatch);
				} else {
		...
		protected obtainFileChangeEmitter(
			ctx: RemoteAgentConnectionContext,
			session: string,
		): Emitter<IFileChangeDto[] | string> {
		...
		listen(ctx: RemoteAgentConnectionContext, event: string, arg?: any): Event<any> {
			...
			if (event === 'readFileStream') {
				...
				const stream = diskFileSystemProvider.readFileStream(
					resource,
					arg[1],
	...
	services.set(
		...
		new SyncDescriptor(ConfigurationService, [environmentService.settingsResource, fileService]),
	);
	...
	instantiationService.invokeFunction((accessor) => {
		...
		channelServer.registerChannel(
			...
			new ExtensionManagementChannel(
				...
				(requestContext) => new URITransformer(rawURITransformerFactory(requestContext)),
			),
		);
		...
		bufferLogService.logger = new SpdLogLogger(
			'main',
			join(environmentService.logsPath, `${RemoteExtensionLogFileName}.log`),
			true,
			...
			try {
				...
				if (
					...
					(await instantiationService.invokeFunction((accessor) =>
						options.handleRequest!(pathname, req, res, accessor, channelServer),
					))
				) {
					return;
				}
				...
				if (pathname === '/vscode-remote-resource') {
					...
					const fsPath =
						typeof filePath === 'string' && URI.from({ scheme: 'file', path: filePath }).fsPath;
					if (!fsPath) {
						return serveError(req, res, 400, 'Bad Request.');
					}
				...
				if (pathname === '/') {
					return handleRoot(
						req,
						res,
						devMode ? options.mainDev || WEB_MAIN_DEV : options.main || WEB_MAIN,
						environmentService,
					);
				}
				...
				if (pathname) {
					...
					if (/^\/static\//.test(pathname)) {
						relativeFilePath = path.normalize(
							decodeURIComponent(pathname.substr('/static/'.length)),
						);
					} else {
		...
		server.on('upgrade', (req: http.IncomingMessage, socket: net.Socket) => {
			if (req.headers['upgrade'] !== 'websocket' || !req.url) {
				logService.error(
					`failed to upgrade for header "${req.headers['upgrade']}" and url: "${req.url}".`,
				);
			...
			const webSocket = new WebSocketNodeSocket(
				...
				if (msg.type === 'error') {
					...
					for (const [token, client] of clients) {
						if (client.management) {
							if (
								client.management.graceTimeReconnection.isScheduled() &&
								!client.management.shortGraceTimeReconnection.isScheduled()
							) {
								logService.info(
									`[${token}] Another connection is established, closing this connection after ${ProtocolConstants.ReconnectionShortGraceTime}ms reconnection timeout.`,
								);
					...
					if (msg.desiredConnectionType === ConnectionType.Management) {
						if (!reconnection) {
							if (client.management) {
								logService.error(
									`[${token}] Falied to connect: management connection is already running.`,
								);
								protocol.sendControl(
									VSBuffer.fromString(
										JSON.stringify({
											type: 'error',
											reason: 'Management connection is already running.',
										} as ErrorMessage),
									),
								);
								safeDisposeProtocolAndSocket(protocol);
							...
							protocol.sendControl(
								VSBuffer.fromString(JSON.stringify({ type: 'ok' } as OKMessage)),
							);
							const graceTimeReconnection = new RunOnceScheduler(() => {
								logService.info(
									`[${token}] Management connection expired after ${ProtocolConstants.ReconnectionGraceTime}ms (grace).`,
								);
								dispose();
							}, ProtocolConstants.ReconnectionGraceTime);
							const shortGraceTimeReconnection = new RunOnceScheduler(() => {
								logService.info(
									`[${token}] Management connection expired after ${ProtocolConstants.ReconnectionShortGraceTime}ms (short grace).`,
								);
								dispose();
							...
							protocol.onSocketClose(() => {
								logService.info(
									`[${token}] Management connection socket is closed, waiting to reconnect within ${ProtocolConstants.ReconnectionGraceTime}ms.`,
								);
								graceTimeReconnection.schedule();
							});
							onDidClientConnectEmitter.fire({
								protocol,
							...
							if (!client.management) {
								logService.error(
									`[${token}] Failed to reconnect: management connection is not running.`,
								);
								protocol.sendControl(
									VSBuffer.fromString(
										JSON.stringify({
											type: 'error',
											reason: 'Management connection is not running.',
										} as ErrorMessage),
									),
								);
								safeDisposeProtocolAndSocket(protocol);
							...
							client.management.protocol.beginAcceptReconnection(
								protocol.getSocket(),
								protocol.readEntireBuffer(),
							);
						...
						if (!reconnection) {
							if (client.extensionHost) {
								logService.error(
									`[${token}] Falied to connect: extension host is already running.`,
								);
								protocol.sendControl(
									VSBuffer.fromString(
										JSON.stringify({
											type: 'error',
											reason: 'Extension host is already running.',
										} as ErrorMessage),
									),
								);
								safeDisposeProtocolAndSocket(protocol);
							...
							protocol.sendControl(
								VSBuffer.fromString(
									JSON.stringify(
										...
										} /* Omit<IExtensionHostConnectionResult, 'protocol'> */,
									),
								),
							);
							const initialDataChunk = Buffer.from(protocol.readEntireBuffer().buffer).toString(
								'base64',
							);
							protocol.dispose();
							socket.pause();
							...
							try {
								...
								const opts: cp.ForkOptions = {
									env: {
										...
										VSCODE_LOG_LEVEL: environmentService.verbose
											? 'trace'
											: environmentService.logLevel,
									},
								...
								if (options.configureExtensionHostForkOptions) {
									instantiationService.invokeFunction((accessor) =>
										options.configureExtensionHostForkOptions!(opts, accessor, channelServer),
									);
								}
								const extensionHost = cp.fork(
								...
								Event.fromNodeEventEmitter<string>(
									extensionHost.stdout!,
									'data',
								)((msg) =>
									logService.info(
										`[${token}][extension host][${extensionHost.pid}][stdout] ${msg}`,
									),
								);
								Event.fromNodeEventEmitter<string>(
									extensionHost.stderr!,
									'data',
								)((msg) =>
									logService.info(
										`[${token}][extension host][${extensionHost.pid}][stderr] ${msg}`,
									),
								);
								extensionHost.on('message', (msg) => {
									if (msg && (<IRemoteConsoleLog>msg).type === '__$console') {
										logService.info(
											`[${token}][extension host][${extensionHost.pid}][__$console] ${
												(<IRemoteConsoleLog>msg).arguments
											}`,
										);
									}
								});
								...
								extensionHost.on('exit', (code: number, signal: string) => {
									...
									if (code !== 0 && signal !== 'SIGTERM') {
										logService.error(
											`[${token}] Extension host exited with code: ${code} and signal: ${signal}.`,
										);
									}
								});
								...
								const readyListener = (msg: any) => {
									if (msg && (<IExtHostReadyMessage>msg).type === 'VSCODE_EXTHOST_IPC_READY') {
										...
										const inflateBytes = Buffer.from(
											webSocket.recordedInflateBytes.buffer,
										).toString('base64');
										extensionHost.send(
											{
								...
								if (options.configureExtensionHostProcess) {
									toDispose = instantiationService.invokeFunction((accessor) =>
										options.configureExtensionHostProcess!(extensionHost, accessor, channelServer),
									);
								}
							...
							if (!client.extensionHost) {
								...
								protocol.sendControl(
									VSBuffer.fromString(
										JSON.stringify({
											type: 'error',
											reason: 'Extension host is not running.',
										} as ErrorMessage),
									),
								);
								safeDisposeProtocolAndSocket(protocol);
							...
							const inflateBytes = Buffer.from(webSocket.recordedInflateBytes.buffer).toString(
								'base64',
							);
							client.extensionHost.send(
								{
